...
function LightningBolt.new(Attachment0, Attachment1, PartCount)
	...
	function module:CreateNewBolt(attach0, attach1)
		local NewBolt = LightningBolt.new(attach0, attach1, 40)
	
		NewBolt.PulseSpeed = 3							-- Bolt arrives at Attachment1 1/PulseSpeed seconds later
		NewBolt.PulseLength = 1							-- Governs speed of how the bolt fades away
		NewBolt.FadeLength = 0.06						-- Lifetime = (self.PulseLength + 1) / self.PulseSpeed
		NewBolt.MinRadius, NewBolt.MaxRadius = 0, 2.4
		NewBolt.Frequency = 1
		NewBolt.AnimationSpeed = 0						-- causes the entire bolt to move (just keep at 0)
		NewBolt.Thickness = 1

		NewBolt.Color = Color3.fromRGB(85, math.random(125, 193), 255)
	end

	for i = 1, 3 do
        self:CreateNewBolt(attach0, attach1)
        task.wait(0.1)
    end
	...
	self.PulseLength = 1000000 -- this will cause the bolt to appear static and unmoving
	self.FadeLength = 0.2
